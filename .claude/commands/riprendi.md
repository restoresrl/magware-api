...
Poi prosegui. Niente Q&A ampie a meno che tu non incontri davvero un dubbio bloccante. Ricorda: niente push automatici, niente endpoint inventati, `npm run check` deve passare pulito prima del commit. Se stai toccando `openapi/magware.yaml`, aggiorna `CHANGELOG-INTERNAL.md` (italiano, dettagliato per-commit, sezione `[Unreleased]`) nello stesso commit. Il `CHANGELOG.md` pubblico (inglese) si aggiorna in modo consolidato sotto `## [Unreleased]` quando il cambio è user-facing significativo; al rilascio `[Unreleased]` diventa `## [X.Y.Z] - YYYY-MM-DD` insieme a tag git e GitHub release.
