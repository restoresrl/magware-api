...
- **`/stocks/{item_code}` non emette 404**: il backend (`magware-refs/n_get_stocks.sru:59-105`) ritorna sempre `200` con `items: []` se l'item code non esiste, anziché un `404`. Comportamento accettabile (l'endpoint è una query, non un fetch by id), e la spec non dichiara `404` qui — ma vale la pena documentarlo nella `description` dell'operation per evitare che gli integratori si aspettino un `404`.
