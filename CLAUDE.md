...
- [ ] **Fase 2bis — Revisione qualitativa della spec** (prima espressione concreta dello scopo primario del progetto — vedi sezione "Scopo primario"). Lavoro **iterativo e dialogico** su `openapi/magware.yaml` per migliorare qualità, chiarezza, coerenza e fruibilità per gli integratori. Non è un check meccanico (quello lo fa già Spectral): è il giudizio di prodotto di Carlo in dialogo con Claude. Si chiude quando Carlo dichiara "soddisfatto" della qualità raggiunta. Track espliciti che la compongono (non si spuntano singolarmente, sono solo guida per non dimenticare aree):
  ...
  **Track aperti** (sequenza raccomandata di lavoro):
  - [ ] **Track 4bis — maxLength audit globale** (abilitato dall'arrivo del DDL `mw21r2.sql` in `magware-refs/`): passata sistematica su tutti i `maxLength:` della spec per verificare che corrispondano alla larghezza reale delle colonne DB. Oggi ci sono decine di `maxLength: 50/100/255/500` dichiarati senza riscontro verificato; un audit chiude la qualità di Track 4 e dà agli integratori vincoli affidabili per la validation client-side. Track autonomo, da fare prima di Track 3 perché interagisce con le description.
  - [ ] **Track 3 — Descrizioni**: passata sistematica per stile coerente (alternanza Indicate/Indicates/Indicating sui boolean), aggiungere description su ~30 property che ne sono prive (es. `package.*`, `package_item.*`, `shipment_details.code`, `adjustment.*`).
