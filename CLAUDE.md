...
- **2026-05-08**: migrato il TLS del sandbox da **Sectigo PositiveSSL** (scaduto 2025-10-28, rinnovo manuale + costo annuale presso reseller) a **Let's Encrypt** autorinnovante. Endpoint pubblico spostato da `https://sandbox.magware.it:9999` a `https://sandbox.magware.it` (porta 443 standard). Stack sul server Windows del sandbox: **win-acme** (`wacs.exe`) come client ACME — challenge HTTP-01 self-hosting (porta 80 libera e già NAT-ata dal firewall), output PEM in `C:\ProgramData\win-acme\certs\`, hook PowerShell `restart-mwp.ps1` che riavvia il servizio Windows `mwp01dbe06621656220` (display name `mwp_mwp`, l'app Elixir/Cowboy in `/dev/mwp`) per fargli ricaricare i nuovi PEM ad ogni rinnovo (~ogni 60 giorni, ~5s di downtime). Cowboy resta esposto direttamente sulla 443 (no reverse proxy davanti). mwp è già env-driven (`MWP_HTTPS_PORTS=443`, `MWP_HTTPS_KEYFILE`, `MWP_HTTPS_CERTFILE`), nessuna modifica al codice mwp. Nessun integratore stava attivamente usando il sandbox al cutover → `:9999` dismessa direttamente, niente bump semver (contratto wire invariato), niente voce nel `CHANGELOG.md` pubblico. Subscription Sectigo da cancellare presso il reseller (stesso pattern dell'annullamento Stoplight del 2026-05-06).
...
- **Changelog (due file distinti)**:
  ...
  - **`CHANGELOG.md`** (inglese, public-facing): contiene **solo entries di release** (formato Keep a Changelog), niente testo introduttivo o di "scope" (le spiegazioni vivono qui in CLAUDE.md). Le entries partono da `### Added / Changed / Deprecated / Removed / Fixed / Security / Known issues` (h3), raggruppate sotto `## [X.Y.Z] - YYYY-MM-DD` (h2) per ogni release. Mentre la prossima release è in lavorazione si usa `## [Unreleased]` in alto, che al rilascio viene rinominato nella versione semver definitiva (`## [X.Y.Z] - YYYY-MM-DD`). Le voci sono **consolidate in inglese**, asciutte, release-level: descrivono solo ciò che aggiunge informazioni nuove o chiarisce punti rilevanti per l'integratore sul **wire** — nuovi endpoint/comportamenti documentati, cambi al modello auth/errori, fix di tipo/formato/nullabilità/vincoli, deprecation. Non appartengono al pubblico: miglioramenti editoriali interni, refactor della struttura YAML, fix di meta-proprietà (license, operationId), rimozione di estensioni proprietarie, aggiornamenti di esempi che non correggono il contratto. Audience: integratori esterni che consumano la spec.
...
6. **Aggiorna `CHANGELOG-INTERNAL.md` ad ogni modifica della spec**. Qualsiasi modifica a `openapi/magware.yaml` va riflessa in `CHANGELOG-INTERNAL.md` (italiano) sotto `## [Unreleased]`, nello stesso commit, con le categorie standard (`Added` / `Changed` / `Deprecated` / `Removed` / `Fixed` / `Security`). Solo cambi al contratto pubblico. Modifiche a tooling/repo/CI/CLAUDE.md non vanno qui (restano nel `git log`). Il `CHANGELOG.md` (inglese, public) si aggiorna solo con le voci che aggiungono informazioni nuove o chiariscono punti rilevanti per l'integratore esterno: nuovi endpoint o comportamenti documentati, cambi al modello di autenticazione/errori, fix di tipo/formato/nullabilità/vincoli nel contratto wire, deprecation. **Non vanno nel pubblico**: miglioramenti editoriali interni (tag descriptions, ordine sidebar, rewrite di info.description), refactor della struttura dello YAML (estrazione schemi condivisi, normalizzazione YAML/quoting), fix di meta-proprietà della spec (license URL, operationId duplicati), rimozione di estensioni proprietarie (x-stoplight-\*, x-internal), aggiornamenti di esempi che non correggono il contratto. Al rilascio `[Unreleased]` viene rinominato nella versione semver definitiva.
